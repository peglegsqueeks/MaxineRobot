...
Optimized for pyrplidar library v0.1.2 - STABLE OPERATION ONLY
...
class StablePyRPLidarA3:
    ...
    def get_info_safe(self):
        """Safely get device information - fixed for pyrplidar API"""
        try:
            if self.lidar and self.is_connected:
                info = self.lidar.get_info()
                # PyRPlidar returns PyRPlidarDeviceInfo object, not dict
                return {
                    'model': getattr(info, 'model', 'A3'),
                    'firmware': getattr(info, 'firmware_version', 'Unknown'),
                    'hardware': getattr(info, 'hardware_version', 'Unknown'),
                    'serialnumber': getattr(info, 'serialnumber', 'A3_Device')
                }
            else:
    ...
    def get_health_safe(self):
        """Safely get device health - fixed for pyrplidar API"""
        try:
            if self.lidar and self.is_connected:
    ...
    def start_scanning(self):
        """Start scanning - simplified for pyrplidar API"""
        try:
            ...
            # Start scanning - pyrplidar handles motor automatically
            self.lidar.start_scan()
            time.sleep(1)  # Wait for scan to stabilize
        ...
        while consecutive_failures < max_failures:
            try:
                ...
                if scan_data and len(scan_data) > 0:
                    ...
                    # Convert pyrplidar format: each point is [quality, angle, distance]
                    processed_scan = []
...
class StableLidarSystem:
    ...
    def data_acquisition_thread(self):
        ...
        try:
            self.lidar = StablePyRPLidarA3(self.port, self.baudrate, timeout=3000)
    ...
    def visualization_thread(self):
        """Visualization thread - MUST run in main thread for matplotlib"""
        pass  # This will be handled in main thread
    
    def update_visualization(self, fig, ax1, ax2, ax3, scan_plot, zone_bars, zone_count_bars, status_text):
        ...
        try:
            ...
            if obstacles:
                ...
                ax1.set_title(f"Maxine's LiDAR - {len(obstacles)} points", fontsize=12)
                ...
                # Update status text
                status_info = f"""PyRPlidar v0.1.2 - STABLE MODE
...
Rear: {zone_distances.get('rear', 'Clear') if zone_distances.get('rear') != float('inf') else 'Clear'}"""
    ...
    def start(self):
        ...
        print("Starting Stable PyRPlidar System...")
        self.running = True
        ...
        if lidar_system.start():
            ...
            while True:
                ...
                if current_time - last_update >= update_interval:
                    if lidar_system.update_visualization(fig, ax1, ax2, ax3, scan_plot, 
                                                       zone_bars, zone_count_bars, status_text):
...
if __name__ == "__main__":
    print("=== Maxine LiDAR Stable Test ===")
