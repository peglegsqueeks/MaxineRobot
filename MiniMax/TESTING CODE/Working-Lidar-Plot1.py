        ...
        while consecutive_failures < max_failures:
            try:
                ...
                if self.start_scanning():
                    consecutive_failures = 0
                    scan_buffer.clear()  # Clear buffer on restart
                    last_angle = None
                    continue
                else:
                    break
...
class SimpleLidarSystem:
    ...
    def stop(self):
        ...
        # Gracefully stop the LiDAR motor
        if self.lidar:
            print("Gracefully stopping LiDAR motor...")
            try:
                self.lidar.lidar.stop()  # Stop scanning
                time.sleep(0.5)
                self.lidar.lidar.set_motor_pwm(0)  # Stop motor
                time.sleep(1)
                print("LiDAR motor stopped successfully")
            except Exception as e:
