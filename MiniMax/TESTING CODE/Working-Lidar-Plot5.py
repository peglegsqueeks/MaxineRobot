...
class SimpleLidarSystem:
    def __init__(self, port='/dev/ttyUSB0', baudrate=256000):
        ...
        # Persistent obstacle map for stable pathfinding
        self.persistent_obstacles = {}  # Dict: {(angle_bin, distance_bin): count}
        self.scan_cycle_count = 0
        self.cycles_before_refresh = 2  # Build map over 2 complete cycles
        self.angle_resolution = 1.0  # 1 degree bins
        self.distance_resolution = 50  # 50mm bins
                ...
                for scan_data in self.lidar.get_scan_data_generator():
                    ...
                    try:
                        if scan_data and len(scan_data) > 0:
                            # Process complete scan for persistent mapping
                            self.process_persistent_scan(scan_data)
    ...
    def process_persistent_scan(self, scan_data):
        """Add scan data to persistent obstacle map"""
        ...
        for quality, angle, distance in scan_data:
            if distance > 0 and quality > 0 and distance < 4000:
                ...
                if key in self.persistent_obstacles:
                    self.persistent_obstacles[key] += 1
                else:
                    self.persistent_obstacles[key] = 1
        
        # Reset map after specified cycles to refresh with new data
        if self.scan_cycle_count >= self.cycles_before_refresh:
            #print(f"Refreshing persistent map after {self.cycles_before_refresh} cycles ({len(self.persistent_obstacles)} obstacles)")
            self.scan_cycle_count = 0
            self.persistent_obstacles.clear()
    
    def get_stable_obstacles(self):
        """Get obstacles that have been seen multiple times (stable/persistent)"""
        stable_obstacles = []
        min_confirmations = max(1, self.cycles_before_refresh // 2)  # Seen in at least half the cycles
        
        for (angle, distance), count in self.persistent_obstacles.items():
...
def simple_lidar_test():
    ...
    matplotlib_polar_axis.set_title("LiDAR - Persistent Map (Stability Mode)", fontsize=11)
