#!/usr/bin/env python3
"""
RPLidar A3 Obstacle Plotting using pyrplidar
"""
import time
import math
import threading
...
class StableRPLidar:
    ...
    def grab_scan(self):
        # Manually read frames one by one
        scan_data = []
        while True:
            frame = self.lidar.read()
            if frame[3] == True:  # startBit detected
                break
        ...
        while True:
            frame = self.lidar.read()
            scan_data.append(frame)
            if frame[3] == True:  # startBit signals end of current scan
                break
