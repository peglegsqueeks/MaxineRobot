#!/usr/bin/env python3
"""
Direct Pipeline LiDAR Test - COMPLETELY BYPASSES CameraSensor Threading
Uses the EXACT same direct DepthAI pipeline as detection_consistency_test.py
Achieves the same ±37px stability by avoiding all threading conflicts
"""
...
# CRITICAL: Import depthai DIRECTLY - no CameraSensor threading
try:
...
class DirectPipelineLidarTest(MaxineBehavior):
    """
    LiDAR Test with DIRECT DepthAI Pipeline - NO CameraSensor Threading
    Uses exact same approach as detection_consistency_test.py for maximum stability
    """
    
    def __init__(self):
        super().__init__("Direct Pipeline LiDAR Test - No Threading")
        ...
        # DIRECT CAMERA SYSTEM - Same as detection_consistency_test.py
        self.device = None
        self.pipeline = None
        ...
        # Camera settings - EXACT same as working standalone test
        self.target_fps = 25
        self.confidence_threshold = 0.4  # Same as detection_consistency_test.py
        self.camera_display_size = (600, 450)
        self.camera_surface = None
        
        # Detection settings - PRESERVED from working test
        ...
        # Z-depth smoothing variables (for compatibility)
    ...
    def initialise(self):
        ...
        """
        print("🎯 Direct Pipeline LiDAR Test initializing...")
        ...
        # CRITICAL: Initialize camera using DIRECT pipeline (no CameraSensor)
        if CAMERA_AVAILABLE:
            self.initialize_direct_camera_pipeline()
        else:
            print("⚠️ Camera not available - running without camera")
        ...
        print("✅ Direct Pipeline LiDAR Test initialized successfully")
    ...
    def initialize_display(self):
        ...
        try:
            ...
            pygame.display.set_caption("MAXINE DIRECT PIPELINE LIDAR TEST")
    ...
    def check_camera_connection(self):
        """Check camera connection - EXACT same as detection_consistency_test.py"""
    ...
    def create_pipeline(self):
        """Create pipeline - EXACT same as detection_consistency_test.py"""
        if not CAMERA_AVAILABLE:
            return None
        ...
        try:
            ...
            # PRESERVED: Exact working camera setup from detection_consistency_test.py
            ...
            detection_nn.setConfidenceThreshold(self.confidence_threshold)  # PRESERVED
    ...
    def initialize_direct_camera_pipeline(self):
        """Initialize camera using DIRECT pipeline - EXACT same as detection_consistency_test.py"""
        try:
            print("📷 Initializing DIRECT camera pipeline (no CameraSensor threading)...")
            ...
            try:
                ...
                print(f"❌ Device connection failed: {str(e)}")
                return False
            
            # Optimize device - EXACT same as detection_consistency_test.py
            try:
            ...
            # Get queues - EXACT same as detection_consistency_test.py
            try:
                ...
                self.detection_queue = None
                self.has_detection = False
                print("❌ Detection queue creation failed")
            ...
            try:
                ...
                self.preview_queue = None
                print("❌ Preview queue creation failed")
            
            # Test camera frames - EXACT same as detection_consistency_test.py
            ...
            if self.camera_initialized:
                ...
                print(f"📊 Detection settings: confidence={self.confidence_threshold}, fps={self.target_fps}")
                return True
            else:
                print("❌ Camera pipeline test failed")
                return False
    ...
    def update_camera_frame(self):
        """Update camera display - EXACT same as detection_consistency_test.py"""
    ...
    def smooth_z_depth(self, raw_z_depth, confidence):
        """Apply confidence-weighted temporal smoothing - PRESERVED from detection_consistency_test.py"""
        try:
    ...
    def process_detections_direct(self):
        """Process detections using DIRECT pipeline - EXACT same logic as detection_consistency_test.py"""
        ...
        try:
            ...
            # CONSISTENCY ANALYSIS - Calculate X midpoint (same as detection_consistency_test.py)
            ...
            if stability_class in self.stability_zones:
            ...
            person_data = {
                'mode_time_elapsed': mode_elapsed,
                'timestamp': time.time(),
    ...
    def initialize_csv_log(self):
        ...
        try:
            ...
            print(f"✅ Direct pipeline consistency logging to: {self.csv_log_filename}")
        except Exception as e:
            print(f"⚠️ CSV initialization failed: {e}")
    
    def log_detection_consistency_to_csv(self, person_data):
        """Log consistency analysis data to CSV"""
        try:
            with open(self.csv_log_filename, 'a', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow([
                    person_data.get('mode_time_elapsed', 0),
                    person_data['timestamp'],
                    person_data.get('frame_number', 0),
                    person_data['x_midpoint_pixels'],
                    person_data['x_midpoint_normalized'],
                    ...
                    person_data['bbox_width'],
                    person_data['bbox_height'],
                    person_data['x_jump_from_previous'],
                    person_data['is_large_jump'],
                    person_data['rolling_variance_pixels'],
                    person_data['rolling_std_dev_pixels'],
                    person_data['rolling_mean_pixels'],
                    person_data['short_term_variance'],
                    person_data['medium_term_variance'],
                    person_data['long_term_variance'],
                    person_data['detection_count'],
                    person_data['consistent_detections'],
                    person_data['large_jumps_count'],
                    person_data['stability_classification'],
                    0.0,  # head_angle_deg - no head tracking for stability
                    False,  # head_tracking_active - disabled for stability
    ...
    def update(self):
        ...
        try:
            # Handle pygame events for ESC exit - CRITICAL for proper exit
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_ESCAPE:
                        print("🔄 ESC pressed - exiting to IDLE mode via py_trees")
                        # Return SUCCESS to trigger terminate() and exit
                        return Status.SUCCESS
                elif event.type == pygame.QUIT:
                    print("🔄 Window closed - exiting to IDLE mode via py_trees")
                    return Status.SUCCESS
            ...
            if self.update_counter % self.display_update_rate == 0:
                try:
                    self.update_display_with_stability_info(person_data, lidar_obstacles)
                except Exception as e:
            ...
            # Return SUCCESS to exit on error
            return Status.SUCCESS
    
    def update_display_with_stability_info(self, person_data, lidar_obstacles):
        """Update display with stability information"""
        if not self.screen:
            return
        ...
        self.draw_info_with_stability(obstacle_count, person_data)
    ...
    def draw_person_detection(self, person_data):
        ...
        try:
            ...
            if z_camera > 0:
                ...
                # Convert x_camera to angle (approximate)
                angle_offset = math.atan2(x_camera, z_camera)
                angle_rad = angle_offset  # Already relative to forward direction
                ...
                stability_class = person_data.get('stability_classification', 'stable')
    ...
    def draw_info_with_stability(self, obstacle_count, person_data):
        """Draw information with stability metrics"""
        try:
            font = pygame.font.Font(None, 36)
            y_offset = 50
            
            # Title
            title_text = font.render("DIRECT PIPELINE LIDAR TEST", True, (0, 255, 255))
            self.screen.blit(title_text, (50, y_offset))
            y_offset += 40
            
            # Detection stability status
            if person_data:
                stability_class = person_data.get('stability_classification', 'unknown')
                std_dev = person_data.get('rolling_std_dev_pixels', 0)
                ...
                stability_text = font.render(f"Stability: {stability_class.upper()} (±{std_dev:.1f}px)", True, stability_color)
                ...
                # Large jumps
                jump_color = (0, 255, 0) if self.large_jumps_count < 5 else (255, 255, 0) if self.large_jumps_count < 15 else (255, 0, 0)
                jump_text = font.render(f"Large Jumps: {self.large_jumps_count}", True, jump_color)
                self.screen.blit(jump_text, (50, y_offset))
                y_offset += 35
            else:
                # No person detected
            ...
            # Mode info
            mode_text = font.render("Detection Method: DIRECT_PIPELINE", True, (0, 255, 255))
    ...
    def terminate(self, new_status):
        ...
        """
        print("🔄 Direct Pipeline LiDAR Test terminating...")
        ...
        try:
            ...
            if self.detection_count > 0:
                ...
                print(f"\n📊 DIRECT PIPELINE CONSISTENCY SUMMARY:")
            ...
            # Close DIRECT camera device (no CameraSensor restoration needed)
            if self.device:
            ...
            print("✅ Direct Pipeline LiDAR Test terminated successfully")
