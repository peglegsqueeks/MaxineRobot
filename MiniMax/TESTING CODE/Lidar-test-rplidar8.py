...
def test_rplidar_a3():
    ...
    try:
        ...
        buffer_clear_interval = 2.0  # Clear buffer every 2 seconds
        ...
        for scan in lidar.iter_scans_safe():
            ...
            # Process scan data quickly
            valid_points = 0
            for quality, angle, distance in scan:
                if 100 < distance < 4000 and quality > 15:  # Stricter filtering
                    angles.append(math.radians(angle))
                    distances.append(distance)
                    valid_points += 1
                    
                    # Limit points per scan to prevent overload
                    if valid_points >= 360:  # Max one point per degree
                        break
