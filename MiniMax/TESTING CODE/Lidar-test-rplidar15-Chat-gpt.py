...
Robustified to handle 'Wrong body size' errors with auto-recovery
"""
import time
import math
import threading
    ...
    def start_lidar_safe(self):
        for attempt in range(3):
            try:
                ...
                self.start_motor()
                time.sleep(2.5)
                self._send_cmd(self._CMD_SCAN)
                return True
            except Exception as e:
                print(f"[LIDAR INIT] Attempt {attempt+1}/3 failed: {e}")
                time.sleep(1)
        raise RuntimeError("Failed to start LIDAR after multiple attempts")
    ...
    try:
        while running_flag[0]:
            try:
                ...
                print(f"[LIDAR EXCEPTION] {e} — attempting recovery")
                lidar.aggressive_buffer_clear()
                lidar.start_lidar_safe()
    finally:
        lidar.stop()
        lidar.stop_motor()
        lidar.disconnect()
